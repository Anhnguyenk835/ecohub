...
class ScheduleBase(BaseModel):
    ...
    command: str = Field(..., description="Specific command to send to the device")
...
class ScheduleCreate(ScheduleBase):
    ...
    daysOfWeek: Optional[List[int]] = Field(None, description="Days of week (1-7 for Monday-Sunday) for weekly schedules")
...
class ScheduleUpdate(BaseModel):
    ...
    command: Optional[str] = Field(None, description="New specific command to send to the device")
    hour: Optional[int] = Field(None, description="New hour in 24-hour format (0-23)")
    minute: Optional[int] = Field(None, description="New minute (0-59)")
    ...
    daysOfWeek: Optional[List[int]] = Field(None, description="New days of week (1-7 for Monday-Sunday) for weekly schedules")
